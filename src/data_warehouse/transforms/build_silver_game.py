...
TRANSFORM_VERSION = "silver-game-0.1.0"
...
def stable_positive_key(value: str) -> int:
    """Create a deterministic 63-bit key so dimension members are stable across reruns."""
...
def parse_time_control(raw_value: str | None) -> dict[str, Any]:
    """Parse Lichess increment notation; unparsed values map to the shared Unknown key."""
...
def rating_difference_bucket_rows() -> list[dict[str, Any]]:
    return [
        {
            "RatingDifferenceBucket_SK": 0,
            "RatingDifferenceBucketLabel": "Unknown",
            "MinDifference": None,
            "MaxDifference": None,
            "SortOrder": 0,
        },
        {
            "RatingDifferenceBucket_SK": 1,
            "RatingDifferenceBucketLabel": "0-49 roughly equal",
            "MinDifference": 0,
            "MaxDifference": 49,
            "SortOrder": 1,
        },
        {
            "RatingDifferenceBucket_SK": 2,
            "RatingDifferenceBucketLabel": "50-99 small edge",
            "MinDifference": 50,
            "MaxDifference": 99,
            "SortOrder": 2,
        },
        {
            "RatingDifferenceBucket_SK": 3,
            "RatingDifferenceBucketLabel": "100-199 meaningful edge",
            "MinDifference": 100,
            "MaxDifference": 199,
            "SortOrder": 3,
        },
        {
            "RatingDifferenceBucket_SK": 4,
            "RatingDifferenceBucketLabel": "200-399 large edge",
            "MinDifference": 200,
            "MaxDifference": 399,
            "SortOrder": 4,
        },
        {
            "RatingDifferenceBucket_SK": 5,
            "RatingDifferenceBucketLabel": "400-599 very large edge",
            "MinDifference": 400,
            "MaxDifference": 599,
            "SortOrder": 5,
        },
        {
            "RatingDifferenceBucket_SK": 6,
            "RatingDifferenceBucketLabel": "600+ extreme mismatch",
            "MinDifference": 600,
            "MaxDifference": None,
            "SortOrder": 6,
        },
...
def validate_rows(
    ...
    if any(row["AbsRatingDifference"] is not None and row["AbsRatingDifference"] < 0 for row in silver_rows):
        status = "fail"
    ...
    if any(
        row["RatingDifferenceBucket_SK"] is not None and row["RatingDifferenceBucket_SK"] not in bucket_keys
        for row in silver_rows
    ):
        status = "fail"
...
def build_silver_outputs(bronze_rows: list[dict[str, Any]], transform_run_id: str) -> dict[str, list[dict[str, Any]]]:
    silver_rows: list[dict[str, Any]] = []
    time_control_members: dict[int, dict[str, Any]] = {}
    termination_members: dict[int, dict[str, Any]] = {}

    for bronze_row in bronze_rows:
        parsed_date = parse_game_date(bronze_row.get("UTCDateRaw"))
        time_control = parse_time_control(bronze_row.get("TimeControlRaw"))
        termination = map_termination(bronze_row.get("TerminationRaw"))

        time_control_members[time_control["TimeControl_SK"]] = time_control
        termination_members[termination["Termination_SK"]] = termination
        ...
        silver_rows.append(
            {
                ...
                "WhiteRating_SK": bronze_row.get("WhiteRating_SK"),
                "BlackRating_SK": bronze_row.get("BlackRating_SK"),
                "AbsRatingDifference": bronze_row.get("AbsRatingDifference"),
                "RatingDifferenceBucket_SK": bronze_row.get("RatingDifferenceBucket_SK"),
    ...
    return {
        "silver_game": silver_rows,
        "dim_time_control": sorted(time_control_members.values(), key=lambda row: (row["TimeControl_SK"], row["TimeControlRaw"])),
        "dim_termination": sorted(termination_members.values(), key=lambda row: (row["Termination_SK"], row["TerminationRaw"])),
        "dim_rating_difference_bucket": rating_difference_bucket_rows(),
    }
...
def transform(args: argparse.Namespace) -> dict[str, Any]:
    ...
    write_parquet(
        ...
        dim_rating_bucket_output,
        DIM_RATING_DIFFERENCE_BUCKET_SCHEMA,
    )
    ...
    if any(status == "fail" for status, _messages in schema_validations):
        validation_status = "fail"
    for _status, messages in schema_validations:
