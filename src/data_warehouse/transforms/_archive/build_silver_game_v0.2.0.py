...
def validate_rows(
    ...
    if any(not isinstance(row["WhiteRating_SK"], int) or row["WhiteRating_SK"] is None for row in silver_rows):
        status = "fail"
        messages.append("One or more WhiteRating_SK values are null or non-integer after normalization.")

    if any(not isinstance(row["BlackRating_SK"], int) or row["BlackRating_SK"] is None for row in silver_rows):
        status = "fail"
